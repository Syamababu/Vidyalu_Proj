...
class TeacherSerializer(serializers.ModelSerializer):
    ...
    def get_report_count(self, obj):
        ...
        count = CourseReport.objects.filter(teacher_id = teacher.teacher_id).count()
        # print(count)
        return count
...
class CounsellorSerializer(serializers.ModelSerializer):
    ...
    def get_report_count(self, obj):
        counsellor = Counsellor.objects.get(id = obj.id)
        count = SessionReport.objects.filter(counsellor_id = counsellor.counsellor_id).count()
        # print(count)
        return count
...
class AdminverifyteacherSerializer(serializers.ModelSerializer):
    """
              serializer for user get the teacher details.
    """

    class Meta:
        model = Teacher
        fields = ["permission_by_admin","admin_comment"]
        # exclude = ("id", "teacher")

class AdminverifycounsellorSerializer(serializers.ModelSerializer):
    """
              serializer for user get the teacher details.
    """
