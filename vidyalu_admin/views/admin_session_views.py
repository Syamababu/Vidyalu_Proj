...
from vidyalu_admin.serializers.admin_session_serializer import SessionDetailSerializer,AdminSessionBookingSerializer,AdminSessionReportGetSerializer
...
class AdminsessionReportGetView(APIView):
    permission_classes = (IsAdminUser,)
    ...
    def post(self,request):
        ...
        report = SessionReport.objects.filter(counsellor_id=counsellor_id)
        if report:
            serialized_rating = AdminSessionReportGetSerializer(report,many=True)
