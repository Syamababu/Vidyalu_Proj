...
class IsStudentCounsellor(BasePermission):
    """
    Allows access only to teacher and student users.
    """
    def has_permission(self, request, view):
        return bool(request.user and request.user.is_authenticated and request.user.is_active and request.user.role == 'counsellor' or 'student')

class IsAdmin(BasePermission):
    def has_permission(self, request, view):
        return bool(request.user.is_superuser)

class IsStudentTeacherCounsellor(BasePermission):
    """
    Allows access only to teacher and student users.
    """
    def has_permission(self, request, view):
        return bool(request.user and request.user.is_authenticated and request.user.is_active and request.user.role == 'teacher' or 'student' or 'counsellor')
