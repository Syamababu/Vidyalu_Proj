...
class TeacherCountCourseView(APIView):
    permission_classes = (IsTeacher,)
    ...
    def get(self, request):
        ...
        if course:
            current_date = timezone.now().date()
            past_course = Course.objects.filter(teacher_id=request.user.id, end_date__lt=current_date).count()
            live_course = Course.objects.filter(teacher_id=request.user.id, start_date__lte=current_date,end_date__gte=current_date).count()
            upcoming_course = Course.objects.filter(teacher_id=request.user.id, start_date__gt=current_date).count()
            total_course=past_course+live_course+upcoming_course
            return api_response(200, "Course count retrieved successfully", [{"total_course":total_course,"past_course": past_course, "live_course": live_course,"upcoming_course": upcoming_course}], status=True)
        else:
            return api_response(200, "No Course found", [{"total_course":0,"past_course": 0, "live_course":0,"upcoming_course": 0}], status=True)



class ActiveCourseDetailView(APIView):
    """
        teacher get the course details.
    """
    permission_classes = (IsTeacher,)
    ...
    def get(self, request):
        course = Course.objects.filter(teacher_id=request.user.id,publish=True)
        if course:
            current_date = timezone.now().date()
            # past_course = course.filter(end_date__lt=current_date)
            # live_course = course.filter(start_date__lte=current_date, end_date__gte=current_date)
            ...
            past_course = Course.objects.filter(teacher_id=request.user.id,publish=True,end_date__lt=current_date)
            live_course = Course.objects.filter(teacher_id=request.user.id,publish=True,start_date__lte=current_date, end_date__gte=current_date)
            upcoming_course = Course.objects.filter(teacher_id=request.user.id,publish=True,start_date__gt=current_date)
            serializer1 = CourseDetailSerializer(instance=past_course, many=True)
            serializer2 = CourseDetailSerializer(instance=live_course, many=True)
            serializer3 = CourseDetailSerializer(instance=upcoming_course, many=True)
