...
class CourseScheduleView(APIView):
    ...
    def post(self,request):
        ...
        course_list = Course.objects.filter(teacher_id = user_id,publish=True).values("id","title","dates_times")
        data = []
        for item in course_list:
            for obj in item['dates_times']:
                obj.update({"teacher_id":user_id,"id":item["id"],"title":item['title'],"class_date_time":self.change_date_time_format(obj['class_date'],obj['class_time'], request.data['time_zone'])})
                data.append(obj)
                ...
                print("before 10 minutes date time: ",(befor_10min_dt.date(),befor_10min_dt.time()))
                print("ctime",todaydt.date(),"--",todaydt.time())
                print("after 2 hours date time",(after_2hr_dt.date(),after_2hr_dt.time()))
                if (todaydt.date() == befor_10min_dt.date() or todaydt.date() == after_2hr_dt.date()) and (todaydt.time()<after_2hr_dt.time()) and (todaydt.time()>befor_10min_dt.time()):
                    status = True
                else:
                    status = False
