...
from student.serializers.student_session_serializer import SessionDetailSerializer,OneSessionDetailSerializer, OneSessionDetailOnlySerializer
    ...
    def post(self, request):
        try:
            ...
            if session:
                ...
                if user.role == 'teacher' or user.role == 'counsellor':
                    serialized_data = serializer.data
                    serialized_data['is_booking'] = True
                    return api_response(200, "Course details retrieved successfully.", [serialized_data], status=True)
...
class StudentSessionDetailsOnlyView(APIView):
    """
           student get the single session details.
    """
    # permission_classes = (IsAuthenticated,)
    ...
    def post(self, request):
        try:
            ...
            if session:
                # user = User.objects.get(id = request.user.id)
                serializer = OneSessionDetailOnlySerializer(session)#context={"user_id":request.user.id}
                # if user.role == 'teacher' or user.role == 'counsellor':
                #     serialized_data = serializer.data
                #     serialized_data['is_booking'] = True
                    # return api_response(200, "Course details retrieved successfully.", [serialized_data], status=True)
