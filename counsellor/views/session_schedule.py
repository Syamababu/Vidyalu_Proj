...
from counsellor.models.session_model import Session,Counsellor
...
class SessionScheduleView(APIView):
    ...
    def get(self,request):
        ...
        session_list = Session.objects.filter(counsellor_id = user_id,publish=True).values("id","title","dates_times")
        data = []
        for item in session_list:
            for obj in item['dates_times']:
                obj.update({"counsellor_id":user_id,"id":item["id"],"title":item['title'],"class_date_time":self.change_date_time_format(obj['session_date'],obj['session_time'])})
                data.append(obj)
        ...
        return api_response(200,"Session Class reterived successfully", data, status = True)