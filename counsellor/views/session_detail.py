...
class counsellorCountSessionView(APIView):

    permission_classes = (IsCounsellor,)

    def get(self, request):
        counsellor_id = request.user.id
        session = Session.objects.filter(counsellor_id=counsellor_id)
        if session:
            current_date = timezone.now().date()
            past_session = Session.objects.filter(counsellor_id=request.user.id, end_date__lt=current_date).count()
            live_session = Session.objects.filter(counsellor_id=request.user.id, start_date__lte=current_date,end_date__gte=current_date).count()
            upcoming_session = Session.objects.filter(counsellor_id=request.user.id, start_date__gt=current_date).count()
            total_session = past_session + live_session + upcoming_session
            return api_response(200,"Session count retrieved successfully",[{"total_session":total_session,"past_session": past_session, "live_session": live_session,
                                 "upcoming_session": upcoming_session}],status=True)

        else:
            return api_response(200, "No Session  found", [{"total_session":0,"past_session": 0, "live_session": 0,"upcoming_session":0}], status=True)
...
class ActiveSessionDetailView(APIView):
    ...
    def get(self, request):
        ...
        if session:
            current_date = timezone.now().date()
            past_session = Session.objects.filter(counsellor_id=request.user.id,publish=True,end_date__lt=current_date)
            live_session = Session.objects.filter(counsellor_id=request.user.id,publish=True,start_date__lte=current_date, end_date__gte=current_date)
            upcoming_session = Session.objects.filter(counsellor_id=request.user.id,publish=True,start_date__gt=current_date)
            serializer1 = SessionDetailSerializer(instance=past_session, many=True)
            serializer2 = SessionDetailSerializer(instance=live_session, many=True)
            serializer3 = SessionDetailSerializer(instance=upcoming_session, many=True)
            # serializer = SessionDetailSerializer(session, many=True)
